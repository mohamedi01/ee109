...
object Main {
  def main(accelUnit: AccelUnit): Unit = {
    ...
    val x1019 = accelUnit.io.memStreams.loads(1).cmd // StreamOut
    ModuleParams.addParams("x1019_p", (x1019.bits.addrWidth, x1019.bits.sizeWidth))  
    val x1021 = accelUnit.io.memStreams.loads(1).data // StreamIn
    ModuleParams.addParams("x1021_p", (x1021.bits.v, x1021.bits.w))  
    // scoped in dram is  
    val x903_b = Wire(new FixedPoint(true, 64, 0))
    x903_b.r := accelUnit.io.argIns(api.B_ptr)
    val x1086 = accelUnit.io.memStreams.loads(2).cmd // StreamOut
    ModuleParams.addParams("x1086_p", (x1086.bits.addrWidth, x1086.bits.sizeWidth))  
    val x1088 = accelUnit.io.memStreams.loads(2).data // StreamIn
    ModuleParams.addParams("x1088_p", (x1088.bits.v, x1088.bits.w))  
    val x1189 = accelUnit.io.memStreams.stores(0).cmd // StreamOut
    ModuleParams.addParams("x1189_p", (x1189.bits.addrWidth, x1189.bits.sizeWidth))  
    val x1190 = accelUnit.io.memStreams.stores(0).data // StreamOut
    ModuleParams.addParams("x1190_p", (x1190.bits.v, x1190.bits.w))  
    val x1191  = accelUnit.io.memStreams.stores(0).wresp // StreamIn
    // scoped in dram is  
    ...
    val RootController = new RootController_kernel(List(x937,x1021,x1088), List(x878_M,x879_N,x880_K), List(x1190), List(x1191), List(x906_c,x900_a,x903_b), List(x1086,x1189,x1019,x935) ,  None, List(), -1, 1, 1, List(1), List(32), breakpoints, instrctrs.toList, rr)
