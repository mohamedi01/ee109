...
object Main {
  def main(accelUnit: AccelUnit): Unit = {
    ...
    val x187 = accelUnit.io.memStreams.stores(0).cmd // StreamOut
    ModuleParams.addParams("x187_p", (x187.bits.addrWidth, x187.bits.sizeWidth))  
    val x188 = accelUnit.io.memStreams.stores(0).data // StreamOut
    ModuleParams.addParams("x188_p", (x188.bits.v, x188.bits.w))  
    val x189  = accelUnit.io.memStreams.stores(0).wresp // StreamIn
    // scoped in dram is  
    ...
    val RootController = new RootController_kernel(List(x160_out), List(x187), List(x189), List(x188) ,  None, List(), -1, 2, 1, List(1), List(32), breakpoints, instrctrs.toList, rr)
