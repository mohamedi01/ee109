...
/** Hierarchy: x964 -> x1002 -> x1703 -> x1704 -> x810 **/
...
class x964_inr_Foreach_kernel(
  ...
  def kernel(): Unit = {
    ...
    class x964_inr_Foreach_concrete(depth: Int)(implicit stack: List[KernelHash]) extends x964_inr_Foreach_module(depth) {
      ...
      val x1813 = Wire(new FixedPoint(true, 32, 0)).suggestName("x1813_x943_rd_x880_D1") 
      x1813.r := getRetimed(x943_rd_x880.r, 1.toInt, io.sigsIn.backpressure & true.B)
      val x1814 = Wire(new FixedPoint(true, 32, 0)).suggestName("x1814_b913_D1") 
      x1814.r := getRetimed(b913.r, 1.toInt, io.sigsIn.backpressure & true.B)
      val x1793 = Wire(new FixedPoint(true, 32, 0)).suggestName("""x1793""")
      x1793.r := Math.fma(x942_sum,x1813,x1814,Some(6.0), ensig0, "x1793").toFixed(x1793, "cast_x1793").r
      val x946 = Wire(new FixedPoint(true, 32, 0)).suggestName("""x946""")
      x946.r := Math.arith_right_shift_div(x1793, 4, Some(0.2), ensig0,"x946").r
      val x947 = Wire(new FixedPoint(true, 32, 0)).suggestName("""x947""")
      x947.r := Math.arith_left_shift(x946, 4, Some(0.2), ensig0,"x947").r
      val x1794 = Wire(new FixedPoint(true, 32, 0)).suggestName("""x1794""")
      x1794.r := Math.arith_left_shift(x946, 6, Some(0.2), ensig0,"x1794").r
      val x949_sub = Wire(new FixedPoint(true, 32, 0)).suggestName("""x949_sub""")
      x949_sub.r := Math.sub(x1793,x947,Some(1.0), ensig0, Truncate, Wrapping, "x949_sub").r
      ...
      x950_rd_x916.toSeq.zip(x916_reg.connectRPort(950, x950_rd_x916_banks, x950_rd_x916_ofs, io.sigsIn.backpressure, x950_rd_x916_en.map(_ && x950_rd_x916_shared_en), false)).foreach{case (a,b) => a.r := b.r}
      val x1815 = Wire(new FixedPoint(true, 32, 0)).suggestName("x1815_x950_rd_x916_D8") 
      x1815.r := getRetimed(x950_rd_x916.r, 8.toInt, io.sigsIn.backpressure & true.B)
      val x951_sum = Wire(new FixedPoint(true, 32, 0)).suggestName("""x951_sum""")
      x951_sum.r := Math.add(x949_sub,x1815,Some(1.0), ensig0, Truncate, Wrapping, "x951_sum").r
      ...
      val x1795 = Wire(new FixedPoint(true, 32, 0)).suggestName("""x1795""")
      x1795.r := Math.arith_left_shift(x953, 6, Some(0.2), ensig0,"x1795").r
      val x956 = Wire(new FixedPoint(true, 64, 0)).suggestName("""x956""")
      x956.r := Math.fix2fix(x1794, true, 64, 0, Some(0.0), ensig0, Truncate, Wrapping, "x956").r
      val x957 = x900_a
      val x1816 = Wire(new FixedPoint(true, 64, 0)).suggestName("x1816_x957_D7") 
      x1816.r := getRetimed(x957.r, 7.toInt, io.sigsIn.backpressure & true.B)
      val x958_sum = Wire(new FixedPoint(true, 64, 0)).suggestName("""x958_sum""")
      x958_sum.r := Math.add(x956,x1816,Some(2.0), ensig0, Truncate, Wrapping, "x958_sum").r
      val x1817 = Wire(new FixedPoint(true, 64, 0)).suggestName("x1817_x958_sum_D1") 
      x1817.r := getRetimed(x958_sum.r, 1.toInt, io.sigsIn.backpressure & true.B)
      val x959_tuple = Wire(UInt(97.W)).suggestName("""x959_tuple""")
      x959_tuple.r := ConvAndCat(true.B,x1795.r,x1817.r)
      val x960 = true.B
      val x1818 = Wire(Bool()).suggestName("x1818_x960_D10") 
      x1818.r := getRetimed(x960.r, 10.toInt, io.sigsIn.backpressure & true.B)
      val x1819 = Wire(Bool()).suggestName("x1819_b941_D10") 
      x1819.r := getRetimed(b941.r, 10.toInt, io.sigsIn.backpressure & true.B)
      x935.valid := (io.sigsIn.datapathEn & io.sigsIn.iiIssue).DS(10.8.toInt.toInt, rr, io.sigsIn.backpressure & true.B) & x1818&x1819 & io.sigsIn.backpressure
      x935.bits.addr := x959_tuple(63,0)
      x935.bits.size := x959_tuple(95,64)
      val x1820 = Wire(new FixedPoint(true, 32, 0)).suggestName("x1820_x951_sum_D1") 
      x1820.r := getRetimed(x951_sum.r, 1.toInt, io.sigsIn.backpressure & true.B)
      val x1821 = Wire(new FixedPoint(true, 32, 0)).suggestName("x1821_x949_sub_D2") 
      x1821.r := getRetimed(x949_sub.r, 2.toInt, io.sigsIn.backpressure & true.B)
      val x962_tuple = Wire(UInt(96.W)).suggestName("""x962_tuple""")
      x962_tuple.r := ConvAndCat(x1820.r,x1821.r,x954.r)
      ...
      x936_fifo.connectWPort(963, x963_enq_x936_banks, x963_enq_x936_ofs, x963_enq_x936_data, x963_enq_x936_en.map(_ && ~io.sigsIn.break && (io.sigsIn.datapathEn & io.sigsIn.iiIssue).DS(10.8.toInt, rr, io.sigsIn.backpressure & true.B) & ~io.sigsIn.break && io.sigsIn.backpressure && true.B & x1819))
      x936_fifo.connectAccessActivesIn(0, ((true.B & x1819)))
    }
