...
object Main {
  def main(accelUnit: AccelUnit): Unit = {
    ...
    val x188 = accelUnit.io.memStreams.stores(0).cmd // StreamOut
    ModuleParams.addParams("x188_p", (x188.bits.addrWidth, x188.bits.sizeWidth))  
    val x189 = accelUnit.io.memStreams.stores(0).data // StreamOut
    ModuleParams.addParams("x189_p", (x189.bits.v, x189.bits.w))  
    val x190  = accelUnit.io.memStreams.stores(0).wresp // StreamIn
    // scoped in dram is  
    val x160_out = Wire(new FixedPoint(true, 64, 0))
    x160_out.r := accelUnit.io.argIns(api.OUT_ptr)
    ...
    val RootController = new RootController_kernel(List(x160_out), List(x188), List(x190), List(x189) ,  None, List(), -1, 2, 1, List(1), List(32), breakpoints, instrctrs.toList, rr)
