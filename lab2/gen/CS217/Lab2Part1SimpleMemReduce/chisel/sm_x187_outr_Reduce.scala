...
/** Hierarchy: x187 -> x119 **/
/** BEGIN None x187_outr_Reduce **/
class x187_outr_Reduce_kernel(
  ...
  val me = this
  val sm = Module(new OuterControl(Pipelined, 2, isFSM = false   , latency = 0.0.toInt, myName = "x187_outr_Reduce_sm")); sm.io <> DontCare
  val iiCtr = Module(new IICounter(1.0.toInt, 2 + _root_.utils.math.log2Up(1.0.toInt), "x187_outr_Reduce_iiCtr"))
  
  abstract class x187_outr_Reduce_module(depth: Int)(implicit stack: List[KernelHash]) extends Module {
  ...
  def connectWires0(module: x187_outr_Reduce_module)(implicit stack: List[KernelHash]): Unit = {
  ...
  def kernel(): Unit = {
    Ledger.enter(this.hashCode, "x187_outr_Reduce")
    implicit val stack = ControllerStack.stack.toList
    class x187_outr_Reduce_concrete(depth: Int)(implicit stack: List[KernelHash]) extends x187_outr_Reduce_module(depth) {
      io.sigsOut := DontCare
      val breakpoints = io.in_breakpoints; breakpoints := DontCare
      val instrctrs = io.in_instrctrs; instrctrs := DontCare
      val rr = io.rr
      val cycles_x187_outr_Reduce = Module(new InstrumentationCounter())
      val iters_x187_outr_Reduce = Module(new InstrumentationCounter())
      cycles_x187_outr_Reduce.io.enable := io.sigsIn.baseEn
      iters_x187_outr_Reduce.io.enable := risingEdge(io.sigsIn.done)
      Ledger.tieInstrCtr(instrctrs.toList, X187_instrctr, cycles_x187_outr_Reduce.io.count, iters_x187_outr_Reduce.io.count, 0.U, 0.U)
      ...
      val x186_inr_Foreach = new x186_inr_Foreach_kernel(List(b166_chain_read_1), List(x161_a_0), List(x170_tmp_0) ,  Some(me), List(x165_ctrchain), 1, 1, 1, List(1), List(32), breakpoints, instrctrs.toList, rr)
      x186_inr_Foreach.sm.io.ctrDone := (x186_inr_Foreach.cchain.head.output.done).DS(1.toInt, rr, io.sigsIn.backpressure & true.B)
      b166_chain.connectStageCtrl((x186_inr_Foreach.done).DS(1.toInt, rr, x186_inr_Foreach.sm.io.backpressure), x186_inr_Foreach.baseEn, 1)
      b168_chain.connectStageCtrl((x186_inr_Foreach.done).DS(1.toInt, rr, x186_inr_Foreach.sm.io.backpressure), x186_inr_Foreach.baseEn, 1)
      x186_inr_Foreach.backpressure := true.B | x186_inr_Foreach.sm.io.doneLatch
      x186_inr_Foreach.forwardpressure := (true.B) && (true.B) | x186_inr_Foreach.sm.io.doneLatch
      x186_inr_Foreach.sm.io.enableOut.zip(x186_inr_Foreach.smEnableOuts).foreach{case (l,r) => r := l}
      x186_inr_Foreach.sm.io.break := false.B
      x186_inr_Foreach.mask := ~x186_inr_Foreach.cchain.head.output.noop & true.B
      x186_inr_Foreach.configure("x186_inr_Foreach", Some(io.sigsIn), Some(io.sigsOut), isSwitchCase = false)
      x186_inr_Foreach.kernel()
    }
    val module = Module(new x187_outr_Reduce_concrete(sm.p.depth)); module.io := DontCare
...
/** END UnrolledReduce x187_outr_Reduce **/
