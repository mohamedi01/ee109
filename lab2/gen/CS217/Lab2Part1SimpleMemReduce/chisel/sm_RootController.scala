...
class RootController_kernel(
  ...
  def kernel(): Unit = {
    ...
    class RootController_concrete(depth: Int)(implicit stack: List[KernelHash]) extends RootController_module(depth) {
      ...
      val x187_outr_Reduce = new x187_outr_Reduce_kernel(List(x165_ctrchain), List(x161_a_0) ,  Some(me), List(x163_ctrchain), 0, 2, 1, List(1), List(32), breakpoints, instrctrs.toList, rr)
      x187_outr_Reduce.sm.io.ctrDone := (x187_outr_Reduce.cchain.head.output.done).DS(1.toInt, rr, io.sigsIn.backpressure & true.B)
      x187_outr_Reduce.backpressure := true.B | x187_outr_Reduce.sm.io.doneLatch
      x187_outr_Reduce.forwardpressure := (true.B) && (true.B) | x187_outr_Reduce.sm.io.doneLatch
      x187_outr_Reduce.sm.io.enableOut.zip(x187_outr_Reduce.smEnableOuts).foreach{case (l,r) => r := l}
      x187_outr_Reduce.sm.io.break := false.B
      x187_outr_Reduce.mask := ~x187_outr_Reduce.cchain.head.output.noop & true.B
      x187_outr_Reduce.configure("x187_outr_Reduce", Some(io.sigsIn), Some(io.sigsOut), isSwitchCase = false)
      x187_outr_Reduce.kernel()
      val x209_outr_UnitPipe_DenseTransfer = new x209_outr_UnitPipe_DenseTransfer_kernel(List(x161_a_0), List(x160_out), List(x188), List(x189), List(x190) ,  Some(me), List(), 1, 3, 3, List(1), List(32), breakpoints, instrctrs.toList, rr)
      x209_outr_UnitPipe_DenseTransfer.backpressure := true.B | x209_outr_UnitPipe_DenseTransfer.sm.io.doneLatch
      x209_outr_UnitPipe_DenseTransfer.forwardpressure := (true.B) && (true.B) | x209_outr_UnitPipe_DenseTransfer.sm.io.doneLatch
      x209_outr_UnitPipe_DenseTransfer.sm.io.enableOut.zip(x209_outr_UnitPipe_DenseTransfer.smEnableOuts).foreach{case (l,r) => r := l}
      x209_outr_UnitPipe_DenseTransfer.sm.io.break := false.B
      x209_outr_UnitPipe_DenseTransfer.mask := true.B & true.B
      x209_outr_UnitPipe_DenseTransfer.configure("x209_outr_UnitPipe_DenseTransfer", Some(io.sigsIn), Some(io.sigsOut), isSwitchCase = false)
      x209_outr_UnitPipe_DenseTransfer.kernel()
    }
