...
object Main {
  def main(accelUnit: AccelUnit): Unit = {
    ...
    val x246 = accelUnit.io.memStreams.stores(0).cmd // StreamOut
    ModuleParams.addParams("x246_p", (x246.bits.addrWidth, x246.bits.sizeWidth))  
    val x247 = accelUnit.io.memStreams.stores(0).data // StreamOut
    ModuleParams.addParams("x247_p", (x247.bits.v, x247.bits.w))  
    val x248  = accelUnit.io.memStreams.stores(0).wresp // StreamIn
    // scoped in dram is  
    val x210_dram = Wire(new FixedPoint(true, 64, 0))
    x210_dram.r := accelUnit.io.argIns(api.DRAM_ptr)
    ...
    val RootController = new RootController_kernel(List(x210_dram), List(x246), List(x248), List(x247) ,  None, List(), -1, 3, 1, List(1), List(32), breakpoints, instrctrs.toList, rr)
