...
object Main {
  def main(accelUnit: AccelUnit): Unit = {
    ...
    val x87_in = accelUnit.io.argIns(api.IN_arg)
    val x88_out = Wire(new MultiArgOut(1)); x88_out.port.map(_.bits := DontCare); x88_out.port.map(_.valid := DontCare); x88_out.output.echo := DontCare
    accelUnit.io.argOuts(accelUnit.io_numArgIOs_reg + 0).port.valid := x88_out.port.map(_.valid).reduce{_||_}
    accelUnit.io.argOuts(accelUnit.io_numArgIOs_reg + 0).port.bits := Mux1H(x88_out.port.map(_.valid), x88_out.port.map(_.bits))
    x88_out.port.map(_.ready := accelUnit.io.argOuts(accelUnit.io_numArgIOs_reg + 0).port.ready)
    x88_out.output.echo := accelUnit.io.argOuts(accelUnit.io_numArgIOs_reg + 0).echo
    val x89_i = accelUnit.io.argIns(api.I_arg)
    val x90_j = accelUnit.io.argIns(api.J_arg)
    ...
    val RootController = new RootController_kernel(List(x88_out), List(x89_i,x87_in,x90_j) ,  None, List(), -1, 1, 1, List(1), List(32), breakpoints, instrctrs.toList, rr)
