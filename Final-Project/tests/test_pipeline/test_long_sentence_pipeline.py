...
LONG_SENTENCES_AUDIO_DIR = PROJECT_ROOT / "data" / "long_sentences" # <-- Changed
LONG_SENTENCES_TRANSCRIPTS_DIR = PROJECT_ROOT / "data" / "transcripts" / "long_sentences" # <-- Changed

# Tolerance for ASR Word Error Rate (adjust as needed for your models)
ASR_WER_TOLERANCE_LONG_SENTENCES = 0.16 # <-- Changed and adjusted value

def get_long_sentence_test_data(): # <-- Renamed
    """
    Scans the data directories and returns a list of tuples (audio_path, transcript_path)
    for all long sentence audio files that have a corresponding ground truth transcript.
    """
    test_data_pairs = []
    if not LONG_SENTENCES_AUDIO_DIR.is_dir() or not LONG_SENTENCES_TRANSCRIPTS_DIR.is_dir():
        print(f"Warning: Audio or transcript directory for long sentences not found. "
              f"Audio: {LONG_SENTENCES_AUDIO_DIR}, Transcripts: {LONG_SENTENCES_TRANSCRIPTS_DIR}")
        return test_data_pairs

    audio_extensions = ["*.wav", "*.mp3", "*.flac", "*.m4a"] # Common audio formats
    for ext in audio_extensions:
        for audio_file in LONG_SENTENCES_AUDIO_DIR.glob(ext):
            transcript_file = LONG_SENTENCES_TRANSCRIPTS_DIR / f"{audio_file.stem}.txt"
            if transcript_file.is_file():
                # Avoid adding duplicates if multiple extensions match the same stem (e.g. audio.wav, audio.mp3)
                if not any(p.values[0].stem == audio_file.stem for p in test_data_pairs):
    ...
    if not test_data_pairs:
        print("Warning: No audio/transcript pairs found for long sentences. "
              "Pipeline tests for long sentences might be skipped by pytest.")
    return test_data_pairs

# --- Test Cases ---

@pytest.mark.parametrize("audio_file_path, truth_transcript_path", get_long_sentence_test_data()) # <-- Renamed function call
def test_long_sentence_full_pipeline_and_nlp_on_truth(audio_file_path: Path, truth_transcript_path: Path): # <-- Renamed test
    ...
    """
    print(f"\n--- Testing Long Sentence Pipeline for: {audio_file_path.name} ---") # <-- Changed log
    ...
    print(f"  ASR Transcript (from Pipeline, first 300 chars): '{asr_transcript_from_pipeline[:300]}...'") # <-- Truncated print
    ...
    print(f"  Ground Truth Transcript (first 300 chars): '{truth_transcript_text[:300]}...'") # <-- Truncated print
    ...
    print(f"  Normalized ASR (Pipeline, first 300 chars): '{norm_asr_pipeline[:300]}...'") # <-- Truncated print
    print(f"  Normalized Truth         (first 300 chars): '{norm_truth[:300]}...'") # <-- Truncated print
    print(f"  Word Error Rate (WER)    : {current_wer:.4f}")
    assert current_wer <= ASR_WER_TOLERANCE_LONG_SENTENCES, ( # <-- Changed tolerance variable
