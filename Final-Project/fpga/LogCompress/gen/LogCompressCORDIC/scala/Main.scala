...
object Main {
  def main(args: Array[String]): Unit = {
    ...
    val x836 = {
      val scanner = new java.util.Scanner(x835)
    ...
    val x840 = Array.tabulate(x836.length){bbb => 
      val b19 = FixedPoint.fromInt(bbb)
      val x838 = x836.apply(b19)
      val x839 = FloatPoint(x838, FltFormat(23,8))
      x839
    }
    val x841 = {
      for (i <- 0 until x840.length) {
        try {
          try {
            x828_inDram(i) = x840(i)
          }
          catch {case err: java.lang.ArrayIndexOutOfBoundsException => 
            System.out.println("[warn] LogCompress.scala:45:11 Memory inputData: Out of bounds read at address " + err.getMessage)
            FloatPoint.invalid(FltFormat(23,8))
          }
        }
        catch {case err: java.lang.ArrayIndexOutOfBoundsException => 
          System.out.println("[warn] LogCompress.scala:45:11 Memory inDram: Out of bounds write at address " + err.getMessage)
        }
      }
    }
    x842_Accel_n.initMem(FixedPoint(BigDecimal("0"),FixFormat(true,32,0)))
    val x843_set_x842 = x842_Accel_n.set(x818)
    x844_Accel_globalLogMax.initMem(FloatPoint("-0.0", FltFormat(23,8)))
    val x845_set_x844 = x844_Accel_globalLogMax.set(x822)
    x846_argIn.initMem(FloatPoint("-0.0", FltFormat(23,8)))
    val x847_set_x846 = x846_argIn.set(x820)
    val x1228_outr_RootController = x1228_outr_RootController_kernel.run()
    val x1199 = new Array[FloatPoint](x818)
    val x1200 = {
      for (i <- 0 until x1199.length) {
        try {
          try {
            x1199(i) = x829_outDram(i)
          }
          catch {case err: java.lang.ArrayIndexOutOfBoundsException => 
            System.out.println("[warn] LogCompress.scala:190:24 Memory outDram: Out of bounds read at address " + err.getMessage)
            FloatPoint.invalid(FltFormat(23,8))
          }
        }
        catch {case err: java.lang.ArrayIndexOutOfBoundsException => 
          System.out.println("[warn] LogCompress.scala:190:24 Memory result: Out of bounds write at address " + err.getMessage)
        }
      }
    }
    val x1201 = {
    ...
    val x1202 = FixedPoint.fromInt(x1199.length)
    val x1205 = {
      val writer = new java.io.PrintWriter(new java.io.FileOutputStream(x1201, true /*append*/))
      (0 until x1202.toInt).foreach{b157 => 
        writer.write("\n")
        val x1203 = x1199.apply(b157)
        val x1204 = x1203.toString
        writer.write(x1204)
      }
      writer.close()
    }
    val x1207 = new Array[FloatPoint](x818)
    val x1208 = {
      for (i <- 0 until x1207.length) {
        try {
          try {
            x1207(i) = x832_rawLogOutDram(i)
          }
          catch {case err: java.lang.ArrayIndexOutOfBoundsException => 
            System.out.println("[warn] LogCompress.scala:195:30 Memory rawLogOutDram: Out of bounds read at address " + err.getMessage)
            FloatPoint.invalid(FltFormat(23,8))
          }
        }
        catch {case err: java.lang.ArrayIndexOutOfBoundsException => 
          System.out.println("[warn] LogCompress.scala:195:30 Memory rawLogResult: Out of bounds write at address " + err.getMessage)
        }
      }
    }
    val x1209 = {
    ...
    val x1210 = FixedPoint.fromInt(x1207.length)
    val x1213 = {
      val writer = new java.io.PrintWriter(new java.io.FileOutputStream(x1209, true /*append*/))
      (0 until x1210.toInt).foreach{b166 => 
        writer.write("\n")
        val x1211 = x1207.apply(b166)
        val x1212 = x1211.toString
        writer.write(x1212)
      }
      writer.close()
    }
  ...
  def printHelp(): Unit = {
    ...
    System.out.print("  -- Args:    <0: n_runtime> <1: dynRange> <2: globalLogMaxIn>\n");
    System.exit(0);
  }
}
