...
object Main {
  def main(accelUnit: AccelUnit): Unit = {
    ...
    val x296 = accelUnit.io.memStreams.loads(0).cmd // StreamOut
    ModuleParams.addParams("x296_p", (x296.bits.addrWidth, x296.bits.sizeWidth))  
    val x298 = accelUnit.io.memStreams.loads(0).data // StreamIn
    ModuleParams.addParams("x298_p", (x298.bits.v, x298.bits.w))  
    // scoped in dram is  
    val x291_inDRAM = Wire(new FixedPoint(true, 64, 0))
    x291_inDRAM.r := accelUnit.io.argIns(api.INDRAM_ptr)
    val x367 = accelUnit.io.memStreams.stores(0).cmd // StreamOut
    ModuleParams.addParams("x367_p", (x367.bits.addrWidth, x367.bits.sizeWidth))  
    val x368 = accelUnit.io.memStreams.stores(0).data // StreamOut
    ModuleParams.addParams("x368_p", (x368.bits.v, x368.bits.w))  
    val x369  = accelUnit.io.memStreams.stores(0).wresp // StreamIn
    // scoped in dram is  
    val x292_outDRAM = Wire(new FixedPoint(true, 64, 0))
    x292_outDRAM.r := accelUnit.io.argIns(api.OUTDRAM_ptr)
    ...
    val RootController = new RootController_kernel(List(x369), List(x298), List(x368), List(x292_outDRAM,x291_inDRAM), List(x296,x367) ,  None, List(), -1, 4, 1, List(1), List(32), breakpoints, instrctrs.toList, rr)
