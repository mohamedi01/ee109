...
class x339_inr_UnitPipe_kernel(
  ...
  def kernel(): Unit = {
    ...
    class x339_inr_UnitPipe_concrete(depth: Int)(implicit stack: List[KernelHash]) extends x339_inr_UnitPipe_module(depth) {
      ...
      val rr = io.rr
      val cycles_x339_inr_UnitPipe = Module(new InstrumentationCounter())
      val iters_x339_inr_UnitPipe = Module(new InstrumentationCounter())
      cycles_x339_inr_UnitPipe.io.enable := io.sigsIn.baseEn
      iters_x339_inr_UnitPipe.io.enable := risingEdge(io.sigsIn.done)
      val stalls_x339_inr_UnitPipe = Module(new InstrumentationCounter())
      val idles_x339_inr_UnitPipe = Module(new InstrumentationCounter())
      stalls_x339_inr_UnitPipe.io.enable := io.sigsIn.baseEn & ~(true.B)
      idles_x339_inr_UnitPipe.io.enable := io.sigsIn.baseEn & ~((x311.valid) && (true.B))
      Ledger.tieInstrCtr(instrctrs.toList, X339_instrctr, cycles_x339_inr_UnitPipe.io.count, iters_x339_inr_UnitPipe.io.count, stalls_x339_inr_UnitPipe.io.count, idles_x339_inr_UnitPipe.io.count)
      val x337 = Wire(Vec(1, Bool())).suggestName("""x337""")
      x311.ready := true.B & (io.sigsIn.datapathEn) 
      (0 until 1).map{ i => x337(i) := x311.bits }
    }
