...
object AppRuntimeModel_dse extends App {
  def build_model(): ControllerModel = {
    val x46 = new ControllerModel(46, OuterControl, Left(Sequenced), CChainModel(Seq()), 0, 1, Ctx("x46", "31", "Accel {", "x46 = AccelScope(Block(Const(())))"))
    val x109_ctrlast = CtrModel(0, 9, 1, 1)
    val x109 = new ControllerModel(109, OuterControl, Left(DenseLoad), List(CChainModel(Seq()), CChainModel(Seq(x109_ctrlast))), 0, 1, Ctx("x109", "35", "inSRAM load inDRAM", "x109 = UnitPipe(Set(),Block(Const(())),None)"), bitsPerCycle = 32.toDouble)
    val x13 = CtrModel(0, 9, 1, 1)
    val x15 = CChainModel(List[CtrModel[_,_,_,_]](x13), Ctx("x15", "37", "Foreach(N by 1) { i =>", "List(x13 = CounterNew(Const(0),Const(9),Const(1),Const(1)))"))
    val x44 = new ControllerModel(44, InnerControl, Left(Pipelined), x15, 67, 1, Ctx("x44", "37", "Foreach(N by 1) { i =>", "x44 = OpForeach(Set(),x15,Block(Const(())),List(b14),None)"))
    val x141_ctrlast = CtrModel(0, 9, 1, 1)
    val x141 = new ControllerModel(141, OuterControl, Left(GatedDenseStore), List(CChainModel(Seq()), CChainModel(Seq(x141_ctrlast))), 0, 1, Ctx("x141", "54", "outDRAM store outSRAM", "x141 = UnitPipe(Set(),Block(Const(())),None)"), bitsPerCycle = 32.toDouble)
    x46.registerChild(x109)
    x46.registerChild(x44)
    x46.registerChild(x141)
    return x46
  }
