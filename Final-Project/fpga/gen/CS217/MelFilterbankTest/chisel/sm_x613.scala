...
class x613_kernel(
  ...
  val me = this
  val sm = Module(new OuterControl(ForkJoin, 2, isFSM = false   , latency = 0.0.toInt, myName = "x613_sm")); sm.io <> DontCare
  val iiCtr = Module(new IICounter(1.0.toInt, 2 + _root_.utils.math.log2Up(1.0.toInt), "x613_iiCtr"))
  
  abstract class x613_module(depth: Int)(implicit stack: List[KernelHash]) extends Module {
  ...
  def connectWires0(module: x613_module)(implicit stack: List[KernelHash]): Unit = {
  ...
  def kernel(): Unit = {
    Ledger.enter(this.hashCode, "x613")
    implicit val stack = ControllerStack.stack.toList
    class x613_concrete(depth: Int)(implicit stack: List[KernelHash]) extends x613_module(depth) {
      io.sigsOut := DontCare
      val breakpoints = io.in_breakpoints; breakpoints := DontCare
      val instrctrs = io.in_instrctrs; instrctrs := DontCare
      val rr = io.rr
      val cycles_x613 = Module(new InstrumentationCounter())
      val iters_x613 = Module(new InstrumentationCounter())
      cycles_x613.io.enable := io.sigsIn.baseEn
      iters_x613.io.enable := risingEdge(io.sigsIn.done)
      Ledger.tieInstrCtr(instrctrs.toList, X613_instrctr, cycles_x613.io.count, iters_x613.io.count, 0.U, 0.U)
    ...
    val module = Module(new x613_concrete(sm.p.depth)); module.io := DontCare
