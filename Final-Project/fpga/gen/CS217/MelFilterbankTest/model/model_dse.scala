...
object AppRuntimeModel_dse extends App {
  def build_model(): ControllerModel = {
    ...
    val x130 = new ControllerModel(130, OuterControl, Left(DenseLoad), List(x76, CChainModel(List(x130_ctrlast))), 0, 1, Ctx("x130", "34", "matSRAM load matDRAM", "x130 = OpForeach(Set(),x76,Block(Const(())),List(b75),None)"), bitsPerCycle = 24.toDouble)
    val x165_ctrlast = CtrModel(0, 3, 1, 1)
    val x165 = new ControllerModel(165, OuterControl, Left(DenseLoad), List(CChainModel(Seq()), CChainModel(Seq(x165_ctrlast))), 0, 1, Ctx("x165", "35", "vecSRAM load vecDRAM", "x165 = UnitPipe(Set(),Block(Const(())),None)"), bitsPerCycle = 24.toDouble)
    ...
    val x43 = new ControllerModel(43, InnerControl, Left(Pipelined), x33, 10, 1, Ctx("x43", "40", "}{_ + _}", "x43 = OpReduce(Set(),x33,x31,Block(x39),Block((x31) => x40),Block((b34,b35) => x41),Block((x31,x41) => x42),None,None,List(b36),None,Fix[TRUE,_16,_8])"))
    x46.registerChild(x43)
    val x197_ctrlast = CtrModel(0, 2, 1, 1)
    val x197 = new ControllerModel(197, OuterControl, Left(GatedDenseStore), List(CChainModel(Seq()), CChainModel(Seq(x197_ctrlast))), 0, 1, Ctx("x197", "44", "outDRAM store outSRAM", "x197 = UnitPipe(Set(),Block(Const(())),None)"), bitsPerCycle = 24.toDouble)
  ...
  override def main(args: Array[String]): Unit = {
    begin("/Users/williambriger/Documents/Stanford/Junior Year /Spring Quarter/EE 109/ee109-1/Final-Project/fpga/./gen/CS217/MelFilterbankTest///results_dse")
