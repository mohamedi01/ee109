...
object AppRuntimeModel_final extends App {
  def build_model(): ControllerModel = {
    val x612 = new ControllerModel(612, OuterControl, Left(Sequenced), CChainModel(Seq()), 0, 1, Ctx("x612", "29", "Accel {", "x612 = AccelScope(Block(Const(())))"))
    val x611 = new ControllerModel(611, OuterControl, Left(ForkJoin), CChainModel(Seq()), 0, 0, Ctx("x611", "82", "stage(ParallelPipe(scala.collection.immutable.Set[Bit](), stageBlock(bundledStms.foreach(visit))))", "x611 = ParallelPipe(Set(),Block(Const(())))"))
    val x476_ctrlast = CtrModel(0, 3, 1, 1)
    val x476 = new ControllerModel(476, OuterControl, Left(DenseLoad), List(CChainModel(Seq()), CChainModel(Seq(x476_ctrlast))), 0, 1, Ctx("x476", "34", "matSRAM load matDRAM", "x476 = UnitPipe(Set(),Block(Const(())),None)"), bitsPerCycle = 32.toDouble)
    val x511_ctrlast = CtrModel(0, 3, 1, 1)
    val x511 = new ControllerModel(511, OuterControl, Left(DenseLoad), List(CChainModel(Seq()), CChainModel(Seq(x511_ctrlast))), 0, 1, Ctx("x511", "35", "vecSRAM load vecDRAM", "x511 = UnitPipe(Set(),Block(Const(())),None)"), bitsPerCycle = 32.toDouble)
    x611.registerChild(x476)
    x611.registerChild(x511)
    ...
    val x537 = new ControllerModel(537, InnerControl, Left(Pipelined), x519, 15, 1, Ctx("x537", "40", "}{_ + _}", "x537 = UnrolledReduce(Set(b515),x519,Block((x516) => Const(())),List(List(b520)),List(List(b521)),None)"))
    ...
    val x574 = new ControllerModel(574, OuterControl, Left(GatedDenseStore), List(CChainModel(Seq()), CChainModel(Seq(x574_ctrlast))), 0, 1, Ctx("x574", "44", "outDRAM store outSRAM", "x574 = UnitPipe(Set(),Block(Const(())),None)"), bitsPerCycle = 32.toDouble)
    x612.registerChild(x611)
    x612.registerChild(x541)
    x612.registerChild(x574)
    return x612
  }
  
  override def main(args: Array[String]): Unit = {
    begin("/Users/mohamedismail/Desktop/EE109/ee109/Final-Project/fpga/./gen/CS217/MelFilterbankTest///results_final")
