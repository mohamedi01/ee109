...
object Main {
  def main(accelUnit: AccelUnit): Unit = {
    ...
    val x254 = accelUnit.io.memStreams.loads(0).cmd // StreamOut
    ModuleParams.addParams("x254_p", (x254.bits.addrWidth, x254.bits.sizeWidth))  
    val x256 = accelUnit.io.memStreams.loads(0).data // StreamIn
    ModuleParams.addParams("x256_p", (x256.bits.v, x256.bits.w))  
    // scoped in dram is  
    val x250_inDRAM = Wire(new FixedPoint(true, 64, 0))
    x250_inDRAM.r := accelUnit.io.argIns(api.INDRAM_ptr)
    val x299 = accelUnit.io.memStreams.stores(0).cmd // StreamOut
    ModuleParams.addParams("x299_p", (x299.bits.addrWidth, x299.bits.sizeWidth))  
    val x300 = accelUnit.io.memStreams.stores(0).data // StreamOut
    ModuleParams.addParams("x300_p", (x300.bits.v, x300.bits.w))  
    val x301  = accelUnit.io.memStreams.stores(0).wresp // StreamIn
    // scoped in dram is  
    ...
    val RootController = new RootController_kernel(List(x299,x254), List(x256), List(x301), List(x300), List(x251_outDRAM,x250_inDRAM) ,  None, List(), -1, 3, 1, List(1), List(32), breakpoints, instrctrs.toList, rr)
