...
object Main {
  def main(accelUnit: AccelUnit): Unit = {
    ...
    val x471 = accelUnit.io.memStreams.loads(0).cmd // StreamOut
    ModuleParams.addParams("x471_p", (x471.bits.addrWidth, x471.bits.sizeWidth))  
    val x473 = accelUnit.io.memStreams.loads(0).data // StreamIn
    ModuleParams.addParams("x473_p", (x473.bits.v, x473.bits.w))  
    // scoped in dram is  
    val x460_inDRAM = Wire(new FixedPoint(true, 64, 0))
    x460_inDRAM.r := accelUnit.io.argIns(api.INDRAM_ptr)
    val x553 = accelUnit.io.memStreams.stores(0).cmd // StreamOut
    ModuleParams.addParams("x553_p", (x553.bits.addrWidth, x553.bits.sizeWidth))  
    val x554 = accelUnit.io.memStreams.stores(0).data // StreamOut
    ModuleParams.addParams("x554_p", (x554.bits.v, x554.bits.w))  
    val x555  = accelUnit.io.memStreams.stores(0).wresp // StreamIn
    // scoped in dram is  
    val x461_realDRAM = Wire(new FixedPoint(true, 64, 0))
    x461_realDRAM.r := accelUnit.io.argIns(api.REALDRAM_ptr)
    val x609 = accelUnit.io.memStreams.stores(1).cmd // StreamOut
    ModuleParams.addParams("x609_p", (x609.bits.addrWidth, x609.bits.sizeWidth))  
    val x610 = accelUnit.io.memStreams.stores(1).data // StreamOut
    ModuleParams.addParams("x610_p", (x610.bits.v, x610.bits.w))  
    val x611  = accelUnit.io.memStreams.stores(1).wresp // StreamIn
    // scoped in dram is  
    val x462_imagDRAM = Wire(new FixedPoint(true, 64, 0))
    x462_imagDRAM.r := accelUnit.io.argIns(api.IMAGDRAM_ptr)
    ...
    val RootController = new RootController_kernel(List(x610,x554), List(x471,x609,x553), List(x473), List(x555,x611), List(x460_inDRAM,x461_realDRAM,x462_imagDRAM) ,  None, List(), -1, 3, 1, List(1), List(32), breakpoints, instrctrs.toList, rr)
